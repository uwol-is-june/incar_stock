...
def _build_prompt(stocks: dict[str, dict]) -> str:
    ...
    lines.append(
        ...
        '  "stocks": {\n'
        '    "<ticker>": "<종목 분석 마크다운 텍스트>"\n'
        '  }\n'
        '}\n\n'
        '종목 분석 마크다운 텍스트 작성 규칙:\n'
        '- 아래 5개 항목을 ### 헤더와 함께 순서대로 작성\n'
        '- ### 가격 / 등락: 당일 주가 흐름과 등락 배경\n'
        '- ### 투자자 동향: 개인·외국인·기관·연기금 순매수 방향과 의미\n'
        '- ### 거래량 · 거래대금: 거래 활성도와 자금 유입/유출 의미\n'
        '- ### 시장 대비 상대 강도: KOSPI·KOSDAQ 흐름 대비 종목 상대 강도\n'
        '- ### 종합 의견: C-level 임원용 한줄 결론\n'
        '- JSON 문자열 내 줄바꿈은 반드시 \\n으로 이스케이프할 것\n'
    )
    return "\n".join(lines)
